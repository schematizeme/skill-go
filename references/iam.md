...
inegociável. Este arquivo é o recorte **backend/Go** do modelo agnóstico
(`schematize-engineering/references/iam.md`): mantém **todas as regras normativas** e
as concretiza no serviço de auth como **microserviço Go** (`<projeto>_auth_go`).
...
- **A autenticação é um serviço próprio, com link próprio e front próprio**, servido em
  **`auth.<domain>`**. **VETADO** apensar o auth ao escopo principal como monolith (nem
  como um pacote `internal/auth` do serviço principal).
...
- **Nunca menos de 2 fatores:** após o bootstrap, **acesso pleno/sensível fica bloqueado
  até enrolar um 2º fator forte** — o middleware do auth barra o token de bootstrap em
  qualquer rota que não seja o enrolamento.
...
- **Motor de relação (ReBAC), ex. OpenFGA/SpiceDB** — hand-rolar authz é onde vazam
  privilégios. O serviço Go **não implementa o motor**: fala com o OpenFGA/SpiceDB via
...
(dedupe de emails, cunha IDs internos ULID/UUIDv7), **força enrolamento de 2º fator no 1º
login pós-migração**, **revoga sessões legadas** e **nunca confia na authz legada**
...
- **F0** Núcleo de identidade (ID imutável, N identificadores, verificação, Resend/Twilio
  plugáveis por interface Go, email OTP always-on) — já como **microserviço Go separado**
  em `auth.<domain>`.
...
- [ ] **≥ 2 fatores** sempre; passkey no núcleo; email OTP (Resend) always-on; Twilio p/ telefone; providers como interfaces Go.
