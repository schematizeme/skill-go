...
func (p *ResendProvider) Send(ctx context.Context, msg Message) error {
	...
	defer resp.Body.Close() //nolint:errcheck // fechar body de resposta não tem tratamento útil
...
// cap <= 0 cai no default 50 — fail-closed: "não configurei" não vira "ilimitado".
func Guarded(p EmailProvider, env Env, testDomains []string, cap int) EmailProvider {
	if cap <= 0 {
		cap = 50
	}
	...
	return &guarded{next: p, env: env, testDomains: normalized, maxPerRun: int64(cap)}
}
...
// é recusa — deny-by-default. A mensagem de erro ENSINA o caminho certo (§37 item 48).
