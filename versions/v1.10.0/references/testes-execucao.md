...
> Parte da skill **schematize-go**. Continuação de `testes.md` (numeração **preservada**, §22.4+): padrão de script de teste, seeds/personas, integração com CI, pentest (§22.7–22.8), fluxo de Q.A. plan-first (disciplina na skill **schematize-qa**) e o Makefile padrão (§23).
...
O fluxo de Q.A. plan-first da casa foi extraído para a skill dedicada **schematize-qa** (`/qa-plan` → `/qa-run`): planeja tudo, pede aprovação, executa faseado/autônomo com watchdog, coleta `summary.json`, trava nos gates. No recorte Go, use `go test -race` e os scripts de teste da skill.
